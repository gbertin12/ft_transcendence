...
const FriendRequests: React.FC = () => {
    ...
    React.useEffect(() => {
        const fetchFriendRequests = async () => {
            axios.get("http://bess-f1r2s10:3000/friends/requests", {
        ...
        <>
            ...
            {friendRequests.map((friendRequest) => (
                ...
                >
                    <Grid xs={1}>
                        <IconX onClick={() => {
                            axios.delete(`http://bess-f1r2s10:3000/friends/requests/${friendRequest.sender_id}`, {
                                withCredentials: true,
                            }).then(() => {
                    ...
                    <Grid xs={1}>
                        <IconCheck onClick={() => {
                            axios.post(`http://bess-f1r2s10:3000/friends/requests/${friendRequest.sender_id}/accept`, {}, {
                                withCredentials: true,
                            }).then(() => {
                                setFriendRequests((requests) => requests.filter((request) => request.sender_id !== friendRequest.sender_id));
                            }).catch((error) => {
