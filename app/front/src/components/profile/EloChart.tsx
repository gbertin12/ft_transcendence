...
async function fetchElo(username: string, date: Date): Promise<number> {
    const res = await fetch(`http://bess-f1r2s5:3000/user/elo/day/${username}?date=${date.toISOString()}`, {
        credentials: "include",
    });
...
async function fetchEloGeneral(date: Date): Promise<number> {
    const res = await fetch(`http://bess-f1r2s5:3000/user/elo/general?date=${date.toISOString()}`);
    const elo = await res.text();
